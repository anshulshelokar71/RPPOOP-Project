...
const PostsWidget = ({ userId, userName, mis, isProfile = false }) => {
  ...
  const getPosts = async () => {
    ...
    if (data[0]) dispatch(setPosts({ posts: data }));
    else dispatch(setPosts({ posts: [] }));
  };

  const enrolStudent = async (props) => {
    const response = await fetch(`${BACKEND_URL}/posts/update`, {
      method: "PATCH",
      headers: {
  ...
  return (
    <>
      {Array.isArray(posts) && posts.length !== 0 ? (
          ...
          <Card sx={{ minWidth: 275, mb: 6 }} key={_id}>
            ...
            <CardActions>
              <Button
                size="small"
                key={_id}
                onClick={() => enrolStudent({ _id })}
              >
                {posts &&
                posts.find((post) => post._id === _id)?.registrations &&
                posts
                  .find((post) => post._id === _id)
                  .registrations.find(
              ...
              <Button
                variant="outlined"
                key={userId}
                onClick={() => handleClickOpen(infoId)}
              >
                Attempt Quiz
              </Button>
              {key && (
        ...
        <div>
          <h2>It seems your Feeds are Empty!!!</h2>
